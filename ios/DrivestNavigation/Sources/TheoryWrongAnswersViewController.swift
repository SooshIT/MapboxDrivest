...
final class TheoryWrongAnswersViewController: UIViewController {
    ...
    private func setupLayout() {
        ...
        config.title = "Start wrong-answer quiz"
        ...
        startQuizButton.addAction(UIAction { [weak self] _ in
            self?.navigationController?.pushViewController(
                TheoryQuizViewController(mode: .wrong, topicId: nil, questionCount: 30),
                animated: true
            )
        }, for: .touchUpInside)
    ...
    private func render() {
        let wrong = progressStore.progress.wrongQueue
        let questions = pack.questions.filter { wrong.contains($0.id) }

        summaryLabel.text = "Wrong-answer queue: \(wrong.count)"
        ...
        if questions.isEmpty {
            let empty = UILabel()
            empty.text = "No wrong-answer questions queued."
